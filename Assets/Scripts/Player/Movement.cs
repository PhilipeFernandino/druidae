...
public class Movement : MonoBehaviour {
    ...
    private void Start() {
        ...
        if (groundCheck == null) Debug.LogError("GroundCheck object not found");
    }

    private void Update() {
        if (inputJump) jumpTimer += Time.deltaTime;
        if (Input.GetButtonDown("Jump")) {
            inputJump = true;
            jumpTimer = 0f;
        }
        verticalAxis = Input.GetAxisRaw("Vertical");
        if (isOnVine && verticalAxis > 0) isGrabbing = true;
    }  
    ...
    private void FixedUpdate() {
        ...
        if (isOnVine && isGrabbing) velocity.x *= speedOnVine.x;
